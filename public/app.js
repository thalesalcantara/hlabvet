...
function publicTrackingHtml(data){
  const c=data?.customer||{},requests=data?.requests||[],pendingQuotes=data?.pendingQuotes||[];
  const pendingHtml=pendingQuotes.length?`<section class="public-pending-quotes"><div class="public-section-head compact"><span>ORÇAMENTOS AGUARDANDO CONFIRMAÇÃO</span><h3>Deseja realizar algum destes exames?</h3><p>Confirme o endereço de coleta para o orçamento virar uma solicitação do laboratório.</p></div>${pendingQuotes.map(q=>`<article class="public-track-card public-pending-quote"><div class="public-track-head"><div><small>${esc(q.quote_number)}</small><h3>${esc(q.patient_name||'Animal')}</h3><span>${fmtDateTime(q.created_at)} • ${fmtMoney(q.total_cents)}</span></div><span class="badge solicitado">Aguardando confirmação</span></div><div class="public-pending-form"><label>Endereço para coleta<input data-pending-address="${q.id}" value="${esc(c.address||'')}" placeholder="Rua, número, bairro e complemento"></label>${q.birth_date?'':`<label>Data de nascimento do animal<input data-pending-birth="${q.id}" type="date" max="${today()}"></label>`}<label>Google Maps (opcional)<input data-pending-map="${q.id}" placeholder="https://maps.app.goo.gl/..."></label><button class="btn primary" type="button" data-public-pending-confirm="${q.id}">CONFIRMAR E CRIAR SOLICITAÇÃO</button></div></article>`).join('')}</section>`:'';
...
async function renderPublicWebsite(focusQuote=false){
  ...
  try{
    ...
    const offers=site.offers||[],partners=site.partners||[],testimonials=site.testimonials||[],tabImages=site.tabImages||{},popupOffer=offers.find(o=>o.offer_type==='promotion'&&Number(o.show_popup)&&o.active!==0)||null;
    $('#courierView').innerHTML=`<div class="hlab-public-site">
      <header class="public-site-header"><a href="/site" class="public-site-logo" data-public-tab="home"><img src="/assets/hlabvet-logo.png" alt="HLab Vet"></a><nav><a href="#inicio" data-public-tab="home">Início</a><a href="#cotacao" data-public-tab="cotacao">Faça sua cotação</a><a href="#acompanhamento" data-public-tab="acompanhamento">Acompanhar exames</a>${offers.length?`<a href="#servicos" data-public-tab="servicos">Serviços e promoções</a>`:''}${partners.length?`<a href="#parceiros" data-public-tab="parceiros">Parceiros</a>`:''}${testimonials.length?`<a href="#depoimentos" data-public-tab="depoimentos">Depoimentos</a>`:''}<a href="#localizacao" data-public-tab="localizacao">Localização</a><a href="#trabalhe" data-public-tab="trabalhe">Trabalhe conosco</a><a href="#contato" data-public-tab="contato">Contato</a></nav><a href="/" class="public-client-login">Área do cliente</a></header>
      <main class="public-site-main">
        ...
        ${testimonials.length?`<section id="depoimentos" class="public-section public-testimonials-section public-site-tab-panel" data-public-panel="depoimentos"><div class="public-section-head"><span>DEPOIMENTOS</span><h2>O que nossos clientes dizem</h2><p>Comentários recebidos pelo WhatsApp, Google e outros canais.</p></div><div class="public-testimonials-grid">${testimonials.map(t=>`<article class="public-testimonial-card"><div class="public-testimonial-shot">${t.screenshotUrl?`<a href="${esc(t.screenshotUrl)}" target="_blank" rel="noopener" aria-label="Abrir print do depoimento de ${esc(t.customer_name)}"><img src="${esc(t.screenshotUrl)}" alt="Print do depoimento de ${esc(t.customer_name)}"></a>`:`<div class="public-testimonial-no-shot"><span>“</span><small>Depoimento do cliente</small></div>`}</div><div class="public-testimonial-body"><div class="public-testimonial-meta"><span class="public-testimonial-source ${esc(t.source||'other')}">${t.source==='whatsapp'?'WhatsApp':t.source==='google'?'Google':'Cliente'}</span></div><h3>${esc(t.customer_name)}</h3><p>${esc(t.testimonial_text||'')}</p></div></article>`).join('')}</div></section>`:''}
    ...
    const hashMap={inicio:'home',home:'home',cotacao:'cotacao',acompanhamento:'acompanhamento',servicos:'servicos',parceiros:'parceiros',depoimentos:'depoimentos',localizacao:'localizacao',trabalhe:'trabalhe',contato:'contato'};
    ...
    const showTracking=async data=>{
      const root=$('#publicTrackingResults');root.innerHTML=publicTrackingHtml(data);$('#publicTrackRefresh')?.classList.remove('hidden');bindPublicResultButtons(root);
      $$('[data-public-pending-confirm]',root).forEach(btn=>btn.addEventListener('click',async()=>{const id=Number(btn.dataset.publicPendingConfirm),address=$(`[data-pending-address="${id}"]`,root)?.value.trim()||'',birthDate=$(`[data-pending-birth="${id}"]`,root)?.value||'',collectionMapUrl=$(`[data-pending-map="${id}"]`,root)?.value.trim()||'';if(!address)return toast('Informe o endereço onde o material será coletado.','error');if($(`[data-pending-birth="${id}"]`,root)&&!birthDate)return toast('Informe a data de nascimento do animal.','error');btn.disabled=true;btn.textContent='CRIANDO SOLICITAÇÃO...';try{const req=await api(`/api/public/quotes/${id}/confirm`,{json:{collectionAddress:address,birthDate,collectionMapUrl}});toast(`Solicitação ${req.protocol} criada.`);await showTracking(await api('/api/public/customer-status'))}catch(e){toast(e.message,'error');btn.disabled=false;btn.textContent='CONFIRMAR E CRIAR SOLICITAÇÃO'}}));
    };
...
async function renderQuotes(){
  ...
  const loadHistory=async()=>{const cid=clientId(),qs=new URLSearchParams();if(cid)qs.set('clientId',cid);if(state.quoteRequisitionId)qs.set('requisitionId',state.quoteRequisitionId);const d=await api(`/api/quotes?${qs}`);$('#quoteHistory').innerHTML=d.quotes.length?d.quotes.map(q=>`<article class="quote-history-item"><div><strong>${esc(q.quote_number)}</strong><small>${esc(q.client_name||'Cliente avulso')} • ${esc(q.patient_name||'Sem paciente')} • ${fmtDateTime(q.updated_at)}</small></div><b>${fmtMoney(q.total_cents)}</b><div class="actions"><button class="btn ghost small" data-q-open="${q.id}">Abrir</button><button class="btn soft small" data-q-print="${q.id}">PDF</button><button class="btn soft small" data-q-share="${q.id}">WhatsApp</button>${internal&&!q.requisition_id?`<button class="btn primary small" data-q-sale="${q.id}">Transformar em venda</button>`:''}${state.me.role==='client'&&!q.requisition_id?`<button class="btn primary small" data-q-request="${q.id}">Criar solicitação</button>`:''}<button class="btn danger small" data-q-delete="${q.id}">Excluir</button></div></article>`).join(''):'<div class="empty-state">Nenhum orçamento salvo.</div>';$$('[data-q-open]').forEach(b=>b.addEventListener('click',()=>openQuoteModal(Number(b.dataset.qOpen))));$$('[data-q-print]').forEach(b=>b.addEventListener('click',()=>printQuoteById(Number(b.dataset.qPrint))));$$('[data-q-share]').forEach(b=>b.addEventListener('click',()=>shareQuoteWhatsApp(Number(b.dataset.qShare))));$$('[data-q-sale]').forEach(b=>b.addEventListener('click',()=>convertQuoteToSaleModal(Number(b.dataset.qSale),loadHistory)));$$('[data-q-request]').forEach(b=>b.addEventListener('click',()=>useQuoteForRequest(Number(b.dataset.qRequest))));$$('[data-q-delete]').forEach(b=>b.addEventListener('click',()=>deleteQuoteWithConfirm(Number(b.dataset.qDelete),loadHistory))) };
...
async function openQuoteModal(id){
  try{
    const d=await api(`/api/quotes/${id}`),q=d.quote;
    const canTransform=state.me.role==='admin'||(state.me.role==='staff'&&state.me.canMakeQuotes);
    modal(`<div class="quote-preview"><div class="quote-preview-head"><img src="/assets/hlabvet-logo.png" alt="HLabVet"><div><small>ORÇAMENTO</small><h2>${esc(q.quote_number)}</h2></div><strong>${fmtMoney(q.total_cents)}</strong></div><div class="quote-preview-meta"><div><small>CLIENTE</small><b>${esc(q.client_name)}</b></div><div><small>PACIENTE</small><b>${esc(q.patient_name||'—')}</b></div><div><small>VALIDADE</small><b>${q.valid_until?fmtDate(q.valid_until):'—'}</b></div></div><div class="table-wrap"><table><thead><tr><th>Item</th><th>Qtd.</th><th>Unitário</th><th>Total</th></tr></thead><tbody>${q.items.map(i=>`<tr><td>${esc(i.description)}<br><small>${i.item_type==='exam'?'Exame':'Serviço'}</small></td><td>${i.quantity}</td><td>${fmtMoney(i.unit_price_cents)}</td><td><strong>${fmtMoney(i.total_cents)}</strong></td></tr>`).join('')}</tbody><tfoot><tr><td colspan="3"><strong>Total</strong></td><td><strong>${fmtMoney(q.total_cents)}</strong></td></tr></tfoot></table></div>${q.notes?`<div class="quote-notes"><b>Observações</b><p>${esc(q.notes)}</p></div>`:''}${q.requisition_id?`<div class="warning-box">Venda já confirmada • Solicitação <b>${esc(q.requisition_protocol||'#'+q.requisition_id)}</b></div>`:''}<div class="actions" style="margin-top:16px"><button class="btn primary" id="quotePdfBtn">PDF / Imprimir</button><button class="btn soft" id="quoteWhatsBtn">Enviar no WhatsApp</button>${canTransform&&!q.requisition_id?'<button class="btn secondary" id="quoteSaleBtn">Transformar em venda</button>':''}${state.me.role==='client'&&q.client_id&&!q.requisition_id?'<button class="btn secondary" id="quoteRequestBtn">Criar solicitação</button>':''}<button class="btn ghost" data-close-modal>Fechar</button></div></div>`);
    $('#quotePdfBtn').addEventListener('click',()=>printQuote(q));
    $('#quoteWhatsBtn').addEventListener('click',()=>shareQuoteWhatsApp(q.id));
    $('#quoteSaleBtn')?.addEventListener('click',()=>{closeModal();convertQuoteToSaleModal(q.id,()=>renderQuotes())});
    $('#quoteRequestBtn')?.addEventListener('click',()=>{closeModal();useQuoteForRequest(q.id)});
  }catch(e){toast(e.message,'error')}
}
...
async function convertQuoteToSaleModal(id,onChanged){
  try{
    const d=await api(`/api/quotes/${id}`),q=d.quote;
    if(q.requisition_id)return toast(`Este orçamento já foi transformado em venda e está na solicitação ${q.requisition_protocol||''}.`,'error');
    const defaultName=q.walk_in_name||q.client_name||'';
    const defaultPhone=q.walk_in_phone||q.client_phone||'';
    const defaultAddress=q.site_customer_address||q.client_address||'';
    const defaultCity=q.site_customer_city||q.client_city||'Natal';
    const defaultState=q.site_customer_state||q.client_state||'RN';
    const defaultZip=q.site_customer_zip||q.client_zip||'';
    modal(`<div class="site-customer-modal"><div class="section-title"><div><h3>Transformar em venda</h3><p>${esc(q.quote_number)} • ${fmtMoney(q.total_cents)}</p></div><span class="badge solicitado">Confirmar dados da coleta</span></div><form id="convertQuoteSaleForm" class="stack"><div class="form-grid"><label class="field span2">Tutor / responsável<input name="name" value="${esc(defaultName)}" required></label><label class="field span2">WhatsApp / telefone<input name="phone" value="${esc(defaultPhone)}" required></label><label class="field span2">Nome do animal<input name="patientName" value="${esc(q.patient_name||'')}" required></label><label class="field">Data de nascimento<input name="birthDate" type="date" max="${today()}" value="${esc(q.patient_birth_date||'')}" required></label><label class="field">Sexo<select name="sex"><option value="">—</option><option value="M" ${q.patient_sex==='M'?'selected':''}>M</option><option value="F" ${q.patient_sex==='F'?'selected':''}>F</option></select></label><label class="field">Espécie<input name="species" value="${esc(q.patient_species||'')}"></label><label class="field">Raça<input name="breed" value="${esc(q.patient_breed||'')}"></label><label class="field span4">Endereço completo da coleta<input name="collectionAddress" value="${esc(defaultAddress)}" placeholder="Rua, número, bairro e complemento" required></label><label class="field span2">Cidade<input name="city" value="${esc(defaultCity)}"></label><label class="field">UF<input name="state" maxlength="2" value="${esc(defaultState)}"></label><label class="field">CEP<input name="zipCode" value="${esc(defaultZip)}"></label><label class="field span4">Localização do Google Maps (opcional)<input name="collectionMapUrl" value="${esc(q.client_map_url||'')}" placeholder="https://maps.app.goo.gl/..."></label></div><div class="warning-box"><b>Ao confirmar a venda:</b> o orçamento será enviado para <b>Solicitações</b> com status <b>Solicitado</b>. O entregador <b>não será atribuído agora</b>; essa etapa será feita depois, dentro da solicitação.</div><div class="actions"><button class="btn primary" type="submit">TRANSFORMAR EM VENDA</button><button type="button" class="btn ghost" data-close-modal>Cancelar</button></div></form></div>`);
    $('#convertQuoteSaleForm').addEventListener('submit',async e=>{
      e.preventDefault();
      const f=new FormData(e.currentTarget),btn=e.currentTarget.querySelector('button[type="submit"]');
      btn.disabled=true;btn.textContent='CONFIRMANDO VENDA...';
      try{
        const r=await api(`/api/quotes/${id}/convert`,{method:'POST',json:{name:f.get('name'),phone:f.get('phone'),patientName:f.get('patientName'),birthDate:f.get('birthDate'),sex:f.get('sex'),species:f.get('species'),breed:f.get('breed'),collectionAddress:f.get('collectionAddress'),city:f.get('city'),state:f.get('state'),zipCode:f.get('zipCode'),collectionMapUrl:f.get('collectionMapUrl')}});
        toast(`${r.message} ${r.protocol||''}`);closeModal();if(onChanged)await onChanged();
      }catch(er){toast(er.message,'error');btn.disabled=false;btn.textContent='TRANSFORMAR EM VENDA'}
    });
  }catch(e){toast(e.message,'error')}
}

async function deleteQuoteWithConfirm(id,onChanged){
  let q;try{q=(await api(`/api/quotes/${id}`)).quote}catch(e){return toast(e.message,'error')}
  const linked=q.requisition_id?`\n\nA solicitação ${q.requisition_protocol||'#'+q.requisition_id} será mantida normalmente.`:'';
  if(!confirm(`Excluir definitivamente o orçamento ${q.quote_number}?${linked}`))return;
  try{const r=await api(`/api/quotes/${id}`,{method:'DELETE'});toast(r.message);if(onChanged)await onChanged()}catch(e){toast(e.message,'error')}
}
...
async function siteTestimonialEditor(testimonial=null){
  const source=testimonial?.source||'whatsapp';
  modal(`<div class="site-editor-modal"><h3>${testimonial?'Editar depoimento':'Novo depoimento'}</h3><p class="muted">Cadastre o nome do cliente, o texto do depoimento e, se quiser, o print original do WhatsApp ou Google. O site padroniza automaticamente o tamanho dos cards.</p><form id="siteTestimonialForm" class="stack"><div class="form-grid"><label class="field span2">Nome do cliente<input name="customerName" value="${esc(testimonial?.customer_name||'')}" required></label><label class="field">Origem<select name="source"><option value="whatsapp" ${source==='whatsapp'?'selected':''}>WhatsApp</option><option value="google" ${source==='google'?'selected':''}>Google</option><option value="other" ${source==='other'?'selected':''}>Outro</option></select></label><label class="field">Ordem<input name="sortOrder" type="number" value="${Number(testimonial?.sort_order||0)}"></label><label class="field span4">O que o cliente escreveu<textarea name="testimonialText" maxlength="1600" required>${esc(testimonial?.testimonial_text||'')}</textarea></label><label class="field span2">Print JPG/PNG/WEBP (opcional)<input name="image" type="file" accept="image/jpeg,image/png,image/webp"><small>O print será exibido inteiro dentro de uma área de tamanho padrão, sem distorcer.</small></label><label class="field">Status<select name="active"><option value="1" ${testimonial?.active!==0?'selected':''}>Ativo</option><option value="0" ${testimonial?.active===0?'selected':''}>Inativo</option></select></label></div><div class="actions"><button class="btn primary">Salvar depoimento</button><button type="button" class="btn ghost" data-close-modal>Cancelar</button></div></form></div>`);
  $('#siteTestimonialForm').addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),body={customerName:fd.get('customerName'),source:fd.get('source'),testimonialText:fd.get('testimonialText'),sortOrder:Number(fd.get('sortOrder')||0),active:fd.get('active')==='1'};try{const r=await api(testimonial?`/api/site/testimonials/${testimonial.id}`:'/api/site/testimonials',{method:testimonial?'PATCH':'POST',json:body});const id=testimonial?.id||r.id,file=fd.get('image');if(file&&file.size){const up=new FormData();up.append('file',file);const res=await fetch(`/api/site/testimonials/${id}/image`,{method:'POST',body:up,credentials:'include'}),x=await res.json();if(!res.ok)throw new Error(x.error||'Falha ao enviar o print.')}toast(testimonial?'Depoimento atualizado.':'Depoimento cadastrado.');closeModal();renderSiteManager()}catch(er){toast(er.message,'error')}});
}
async function deleteSitePartner(id){
  if(!confirm('Excluir este parceiro? A logo também será removida do site.'))return;
  try{const r=await api(`/api/site/partners/${id}`,{method:'DELETE'});toast(r.message);renderSiteManager()}catch(e){toast(e.message,'error')}
}

async function deleteSiteTestimonial(id){
  if(!confirm('Excluir este depoimento do site?'))return;
  try{const r=await api(`/api/site/testimonials/${id}`,{method:'DELETE'});toast(r.message);renderSiteManager()}catch(e){toast(e.message,'error')}
}
...
async function renderSiteManager(){
  ...
  const [offerData,partnerData,testimonialData]=await Promise.all([api('/api/site/offers'),api('/api/site/partners'),api('/api/site/testimonials')]);
  const offers=offerData.offers||[],partners=partnerData.partners||[],testimonials=testimonialData.testimonials||[],admin=state.me.role==='admin',siteUrl=`${location.origin}/site`,quoteUrl=`${location.origin}/orcamento`;
  ...
  $('#newSiteTestimonial')?.addEventListener('click',()=>siteTestimonialEditor());$('#newSiteTestimonial2')?.addEventListener('click',()=>siteTestimonialEditor());
  ...
  $$('[data-delete-site-partner]').forEach(b=>b.addEventListener('click',()=>deleteSitePartner(Number(b.dataset.deleteSitePartner))));
  $$('[data-edit-site-testimonial]').forEach(b=>b.addEventListener('click',()=>siteTestimonialEditor(testimonials.find(x=>Number(x.id)===Number(b.dataset.editSiteTestimonial)))));
  $$('[data-delete-site-testimonial]').forEach(b=>b.addEventListener('click',()=>deleteSiteTestimonial(Number(b.dataset.deleteSiteTestimonial))));
