...
async function renderPublicWebsite(focusQuote=false){
  ...
  try{
    const site=await api('/api/public/site');if(!site.enabled){$('#courierView').innerHTML=`<div class="public-site-offline"><img src="/assets/hlabvet-logo.png"><h1>Site temporariamente indisponível</h1><p>Entre em contato com o HLab Vet pelos canais de atendimento.</p><a class="btn primary" href="/">Área do cliente</a></div>`;return}
    const st=site.settings||{},phone=String(st.whatsappPhone||'').replace(/\D/g,''),wa=phone?`https://wa.me/${phone.startsWith('55')?phone:`55${phone}`}`:'#contato';let quoteData=null;try{if(site.quote?.enabled)quoteData=await api('/api/public/quote-catalog')}catch{}
    ...
    $('#courierView').innerHTML=`<div class="hlab-public-site"><header class="public-site-header"><a href="/site" class="public-site-logo"><img src="/assets/hlabvet-logo.png" alt="HLab Vet"></a><nav><a href="#cotacao">Faça sua cotação</a><a href="#servicos">Serviços e promoções</a><a href="#localizacao">Localização</a><a href="#trabalhe">Trabalhe conosco</a><a href="#contato">Contato</a></nav><a href="/" class="public-client-login">Área do cliente</a></header><main><section class="public-hero"><div class="public-hero-copy"><span>HLAB VET • DIAGNÓSTICOS VETERINÁRIOS</span><h1>${esc(st.headline||'Diagnóstico veterinário com agilidade, cuidado e confiança')}</h1><p>${esc(st.subheadline||'')}</p><div class="public-hero-actions"><a class="public-cta primary" href="#cotacao">Faça sua cotação</a>${phone?`<a class="public-cta secondary" href="${wa}" target="_blank" rel="noopener">Falar no WhatsApp</a>`:''}</div><div class="public-trust-row"><div><b>Exames</b><small>Catálogo atualizado</small></div><div><b>Coleta</b><small>Fluxo integrado</small></div><div><b>Resultados</b><small>Acesso digital</small></div></div></div><div class="public-hero-visual"><div class="public-hero-mark"><img src="/assets/hlabvet-logo.png"><span>Ciência a favor da vida animal</span></div></div></section>${offers.length?`<section id="servicos" class="public-section public-offers-section"><div class="public-section-head"><span>DESTAQUES</span><h2>Serviços, produtos e promoções</h2><p>Conteúdos selecionados pelo HLab Vet.</p></div><div class="public-offer-grid">${offers.map(o=>`<article class="public-offer-card">${o.imageUrl?`<div class="public-offer-image"><img src="${esc(o.imageUrl)}" alt="${esc(o.title)}">${o.badge?`<span>${esc(o.badge)}</span>`:''}</div>`:`<div class="public-offer-image placeholder"><img src="/assets/hlabvet-logo.png" alt="HLab Vet">${o.badge?`<span>${esc(o.badge)}</span>`:''}</div>`}<div class="public-offer-body"><small>${o.offer_type==='product'?'PRODUTO':o.offer_type==='promotion'?'PROMOÇÃO':'SERVIÇO'}</small><h3>${esc(o.title)}</h3><p>${esc(o.description||'')}</p>${o.promo_price_cents!=null?`<div class="public-offer-price"><del>${o.price_cents!=null?fmtMoney(o.price_cents):''}</del><strong>${fmtMoney(o.promo_price_cents)}</strong></div>`:o.price_cents!=null?`<div class="public-offer-price"><strong>${fmtMoney(o.price_cents)}</strong></div>`:''}<a href="${phone?`${wa}?text=${encodeURIComponent(`Olá! Gostaria de saber mais sobre ${o.title}.`)}`:'#contato'}" ${phone?'target="_blank" rel="noopener"':''} class="public-card-cta">${esc(o.button_text||'Quero saber mais')}</a></div></article>`).join('')}</div></section>`:''}<section id="cotacao" class="public-section public-quote-section"><div class="public-section-head"><span>FAÇA SUA COTAÇÃO</span><h2>Monte seu orçamento de exames</h2><p>Pesquise, selecione os exames disponíveis e envie a cotação diretamente para a equipe HLab Vet.</p></div>${site.quote?.enabled&&quoteData?`<div class="public-quote-layout"><aside class="public-quote-contact"><h3>Seus dados</h3><label>Nome / clínica<input id="publicQuoteName" required placeholder="Seu nome ou clínica"></label><label>WhatsApp<input id="publicQuotePhone" required inputmode="tel" placeholder="(84) 99999-9999"></label><label>E-mail (opcional)<input id="publicQuoteEmail" type="email" placeholder="voce@email.com"></label><label>Paciente / animal (opcional)<input id="publicQuotePatient" placeholder="Nome do animal"></label><div class="public-quote-summary"><span><small>Itens selecionados</small><b id="publicQuoteCount">0</b></span>${quoteData.showPrices?`<span><small>Total</small><strong id="publicQuoteTotal">R$ 0,00</strong></span>`:''}</div><button class="public-submit-quote" id="publicQuoteSubmit">ENVIAR COTAÇÃO AO HLAB VET</button><div id="publicQuoteSuccess"></div></aside><div class="public-quote-catalog"><div class="public-quote-search"><span>⌕</span><input id="publicQuoteSearch" placeholder="Informe o nome do exame"></div><div class="public-exam-list" id="publicExamGrid">${[...groups.entries()].map(([cat,items])=>`<section class="public-exam-category"><h3>${esc(cat)}</h3>${items.map(x=>`<label class="public-exam-line" data-search="${esc((x.examName+' '+cat).toLowerCase())}"><input type="checkbox" data-public-exam="${x.examCode}" data-name="${esc(x.examName)}" data-price="${x.priceCents||0}" ${x.priceCents==null?'disabled':''}><span>${esc(x.examName)}</span>${quoteData.showPrices?`<strong>${x.priceCents==null?'Consultar preço':fmtMoney(x.priceCents)}</strong>`:''}</label>`).join('')}</section>`).join('')}</div>${quoteData.showServices&&quoteData.services?.length?`<section class="public-exam-category public-extra-services"><h3>Serviços adicionais</h3>${quoteData.services.map(x=>`<label class="public-exam-line"><input type="checkbox" data-public-service="${x.id}" data-name="${esc(x.name)}" data-price="${x.price_cents}"><span>${esc(x.name)}${x.description?`<small>${esc(x.description)}</small>`:''}</span>${quoteData.showPrices?`<strong>${fmtMoney(x.price_cents)}</strong>`:''}</label>`).join('')}</section>`:''}</div></div>`:`<div class="public-offline-inline"><h3>Cotação online temporariamente desativada</h3><p>O site continua disponível. Para orçamento, fale com o laboratório.</p>${phone?`<a href="${wa}" target="_blank" class="public-cta primary">Falar no WhatsApp</a>`:''}</div>`}</section><section id="localizacao" class="public-section public-location-section"><div class="public-location-card"><div><span>ONDE ESTAMOS</span><h2>Localização</h2><p>${esc(st.address||'Cadastre o endereço no painel do site.')}</p>${st.mapUrl?`<a href="${esc(st.mapUrl)}" target="_blank" rel="noopener" class="public-cta primary">Abrir localização exata no Google Maps</a>`:''}</div><div class="public-map-visual"><div class="map-pin">⌖</div><strong>HLab Vet</strong><small>${esc(st.address||'Natal/RN')}</small></div></div></section><section id="trabalhe" class="public-section public-careers-section"><div class="public-careers-card"><div><span>TRABALHE CONOSCO</span><h2>Faça parte da nossa equipe</h2><p>Envie seu currículo por e-mail. O destinatário é configurado pelo laboratório no próprio sistema.</p>${st.careersEmail?`<a class="public-cta primary" href="mailto:${encodeURIComponent(st.careersEmail)}?subject=${encodeURIComponent('Currículo - Trabalhe conosco HLab Vet')}&body=${encodeURIComponent('Olá! Gostaria de enviar meu currículo para oportunidades no HLab Vet. Favor anexar o currículo a este e-mail.')}">Enviar currículo por e-mail</a><small>Envie para: ${esc(st.careersEmail)} • anexe o currículo antes de enviar.</small>`:'<small>O e-mail de currículos ainda não foi configurado.</small>'}</div><div class="public-careers-art"><div>HLAB VET</div><span>Talento + cuidado + ciência</span></div></div></section><section id="contato" class="public-section public-contact-section"><div class="public-section-head"><span>CONTATO</span><h2>Fale com o HLab Vet</h2></div><div class="public-contact-grid">${phone?`<a href="${wa}" target="_blank"><b>WhatsApp</b><span>${esc(st.whatsappPhone)}</span></a>`:''}${st.contactEmail?`<a href="mailto:${esc(st.contactEmail)}"><b>E-mail</b><span>${esc(st.contactEmail)}</span></a>`:''}<a href="/"><b>Área do cliente</b><span>Acompanhar solicitações e resultados</span></a></div></section></main><footer class="public-site-footer"><img src="/assets/hlabvet-logo.png"><span>${esc(st.companyName||'HLab Vet Resultados')}</span><small>Diagnósticos veterinários • ${new Date().getFullYear()}</small></footer>${phone?`<a class="public-floating-whatsapp" href="${wa}" target="_blank" rel="noopener" aria-label="WhatsApp">WA</a>`:''}</div>`;
    if(site.quote?.enabled&&quoteData){const selectedItems=()=>{const examCodes=$$('[data-public-exam]:checked').map(x=>x.dataset.publicExam),services=$$('[data-public-service]:checked').map(x=>({id:Number(x.dataset.publicService),quantity:1}));return {examCodes,services}};const update=()=>{let n=0,total=0;$$('[data-public-exam]:checked').forEach(x=>{n++;total+=Number(x.dataset.price||0)});$$('[data-public-service]:checked').forEach(x=>{n++;total+=Number(x.dataset.price||0)});$('#publicQuoteCount').textContent=n;if($('#publicQuoteTotal'))$('#publicQuoteTotal').textContent=fmtMoney(total)};$$('[data-public-exam],[data-public-service]').forEach(x=>x.addEventListener('change',update));$('#publicQuoteSearch')?.addEventListener('input',e=>{const q=e.target.value.toLowerCase().trim();$$('.public-exam-line[data-search]').forEach(x=>x.classList.toggle('hidden',q&&!x.dataset.search.includes(q)))});$('#publicQuoteSubmit')?.addEventListener('click',async()=>{const name=$('#publicQuoteName').value.trim(),p=$('#publicQuotePhone').value.trim(),sel=selectedItems();if(!name)return toast('Informe seu nome ou clínica.','error');if(!p)return toast('Informe seu WhatsApp.','error');if(!sel.examCodes.length&&!sel.services.length)return toast('Selecione pelo menos um exame ou serviço.','error');const b=$('#publicQuoteSubmit');b.disabled=true;b.textContent='ENVIANDO...';try{const r=await api('/api/public/quotes',{json:{name,phone:p,email:$('#publicQuoteEmail').value,patientName:$('#publicQuotePatient').value,...sel}}),q=r.quote;$('#publicQuoteSuccess').innerHTML=`<div class="public-quote-success"><b>✓ Cotação enviada</b><span>${esc(q.quote_number)} • ${fmtMoney(q.total_cents)}</span><small>A equipe HLab Vet já consegue visualizar este orçamento no sistema e poderá entrar em contato pelo WhatsApp informado.</small><div><button class="btn ghost small" id="publicQuotePrint">PDF / Imprimir</button>${phone?`<a class="btn secondary small" href="${wa}?text=${encodeURIComponent(`Olá! Acabei de enviar a cotação ${q.quote_number} pelo site HLab Vet.`)}" target="_blank">WhatsApp</a>`:''}</div></div>`;$('#publicQuotePrint')?.addEventListener('click',()=>printQuote(q));b.textContent='COTAÇÃO ENVIADA'}catch(e){toast(e.message,'error');b.disabled=false;b.textContent='ENVIAR COTAÇÃO AO HLAB VET'}});update();}
    if(focusQuote)setTimeout(()=>$('#cotacao')?.scrollIntoView({behavior:'smooth',block:'start'}),80);
...
async function setupOnlineQuoteTabs(){
  ...
  $('#content').insertAdjacentHTML('beforeend',`<section id="onlineQuotesPanel" class="card online-quotes-panel hidden"><div class="section-title"><div><h3>Orçamentos solicitados pelo site</h3><p>Novas cotações entram aqui automaticamente para o laboratório, técnico ou vendedor autorizado acompanhar e entrar em contato.</p></div></div><form id="onlineQuoteFilters" class="online-quote-filters"><label>De<input name="from" type="date" value="${first}"></label><label>Até<input name="to" type="date" value="${today()}"></label><label>Status<select name="leadStatus"><option value="">Todos</option><option value="new">Novos</option><option value="contacted">Contatados</option><option value="converted">Convertidos</option><option value="closed">Encerrados</option></select></label><button class="btn primary">Buscar</button></form><div id="onlineQuoteList"><div class="empty-state">Carregando...</div></div></section>`);
  const load=async()=>{const qs=new URLSearchParams(new FormData($('#onlineQuoteFilters')));qs.set('source','online');for(const[k,v]of[...qs])if(!v)qs.delete(k);const d=await api(`/api/quotes?${qs}`);$('#onlineQuoteBadge').textContent=Number(d.onlineNewCount||0);$('#onlineQuoteList').innerHTML=d.quotes.length?`<div class="table-wrap"><table class="online-quote-table"><thead><tr><th>Data</th><th>Cliente</th><th>Contato</th><th>Paciente</th><th>Total</th><th>Status</th><th>Ações</th></tr></thead><tbody>${d.quotes.map(q=>`<tr><td>${fmtDateTime(q.created_at)}</td><td><strong>${esc(q.walk_in_name||q.client_name||'Cliente')}</strong>${q.lead_email?`<br><small>${esc(q.lead_email)}</small>`:''}<br><small>${esc(q.quote_number)}</small></td><td>${esc(q.walk_in_phone||'—')}</td><td>${esc(q.patient_name||'—')}</td><td><strong>${fmtMoney(q.total_cents)}</strong></td><td><span class="badge ${onlineLeadClass(q.lead_status)}">${onlineLeadLabel(q.lead_status)}</span></td><td><div class="actions"><button class="btn ghost small" data-online-open="${q.id}">Abrir</button>${q.walk_in_phone?`<button class="btn secondary small" data-online-whats="${q.id}" data-phone="${esc(q.walk_in_phone)}" data-name="${esc(q.walk_in_name||'')}">WhatsApp</button>`:''}<select class="online-status-select" data-online-status="${q.id}"><option value="new" ${(q.lead_status||'new')==='new'?'selected':''}>Novo</option><option value="contacted" ${q.lead_status==='contacted'?'selected':''}>Contatado</option><option value="converted" ${q.lead_status==='converted'?'selected':''}>Convertido</option><option value="closed" ${q.lead_status==='closed'?'selected':''}>Encerrado</option></select></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty-state">Nenhum orçamento online encontrado no período.</div>';$$('[data-online-open]').forEach(b=>b.addEventListener('click',()=>openQuoteModal(Number(b.dataset.onlineOpen))));$$('[data-online-whats]').forEach(b=>b.addEventListener('click',async()=>{const id=Number(b.dataset.onlineWhats),phone=String(b.dataset.phone||'').replace(/\D/g,''),name=b.dataset.name||'';window.open(`https://wa.me/${phone?`55${phone.replace(/^55/,'')}`:''}?text=${encodeURIComponent(`Olá${name?` ${name}`:''}! Recebemos sua cotação pelo site do HLab Vet e gostaríamos de confirmar se deseja realizar os exames/serviços.`)}`,'_blank','noopener');try{await api(`/api/quotes/${id}/lead-status`,{method:'PATCH',json:{status:'contacted'}});await load()}catch{}}));$$('[data-online-status]').forEach(sel=>sel.addEventListener('change',async()=>{try{const r=await api(`/api/quotes/${sel.dataset.onlineStatus}/lead-status`,{method:'PATCH',json:{status:sel.value}});toast(r.message);await load()}catch(e){toast(e.message,'error')}}));};
