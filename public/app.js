...
async function renderPublicQuoteSite(){
  stopLabAlerts();$('#loginView').classList.add('hidden');$('#appView').classList.add('hidden');$('#courierView').classList.remove('hidden');document.body.classList.remove('courier-mobile-mode');document.body.classList.add('public-quote-mode');
  try{const cfg=await api('/api/public/quote-site');if(!cfg.enabled){$('#courierView').innerHTML=`<div class="public-quote-shell"><header><img src="/assets/hlabvet-logo.png"><div><h1>Orçamento online</h1><p>HLab Vet Resultados</p></div></header><main><div class="public-offline"><h2>Orçamento online temporariamente desativado</h2><p>Entre em contato com o laboratório para solicitar seu orçamento.</p><a class="btn primary" href="/">Acessar sistema</a></div></main></div>`;return}
    const d=await api('/api/public/quote-catalog'),groups=new Map();for(const x of d.exams){if(!groups.has(x.category))groups.set(x.category,[]);groups.get(x.category).push(x)};
    $('#courierView').innerHTML=`<div class="public-quote-shell"><header><img src="/assets/hlabvet-logo.png"><div><span>ORÇAMENTO ONLINE</span><h1>Exames veterinários</h1><p>Selecione somente exames cadastrados pelo HLab Vet e veja o orçamento na hora.</p></div><a href="/" class="btn ghost small">Área do cliente</a></header><main><section class="public-quote-intro"><div><label>Nome / clínica (opcional)<input id="publicQuoteName" placeholder="Seu nome ou clínica"></label><label>Paciente / animal (opcional)<input id="publicQuotePatient" placeholder="Nome do animal"></label></div><label>Pesquisar exame<input id="publicQuoteSearch" placeholder="Digite o nome do exame..."></label><small>Não encontrou? Entre em contato com o laboratório. Exames não podem ser digitados manualmente.</small></section><section class="public-exams" id="publicExamGrid">${[...groups.entries()].map(([cat,items])=>`<article class="public-exam-group"><h2>${esc(cat)}</h2>${items.map(x=>`<label class="public-exam-item" data-search="${esc((x.examName+' '+cat).toLowerCase())}"><input type="checkbox" data-public-exam="${x.examCode}" data-name="${esc(x.examName)}" data-price="${x.priceCents||0}"><span>${esc(x.examName)}</span>${d.showPrices?`<strong>${fmtMoney(x.priceCents)}</strong>`:''}</label>`).join('')}</article>`).join('')}</section>${d.showServices&&d.services.length?`<section class="public-services"><h2>Serviços adicionais</h2>${d.services.map(x=>`<div class="public-service-item"><label><input type="checkbox" data-public-service="${x.id}" data-name="${esc(x.name)}" data-price="${x.price_cents}"><span><b>${esc(x.name)}</b>${x.description?`<small>${esc(x.description)}</small>`:''}</span></label>${d.showPrices?`<strong>${fmtMoney(x.price_cents)}</strong>`:''}</div>`).join('')}</section>`:''}<div class="public-total"><div><small>ITENS SELECIONADOS</small><span id="publicQuoteCount">0</span></div>${d.showPrices?`<div><small>VALOR TOTAL</small><strong id="publicQuoteTotal">R$ 0,00</strong></div>`:''}<div class="actions"><button class="btn primary" id="publicQuotePdf">PDF / Imprimir</button><button class="btn secondary" id="publicQuoteWhats">WhatsApp</button></div></div></main></div>`;
    const selectedItems=()=>{const items=[];$$('[data-public-exam]:checked').forEach(x=>items.push({item_type:'exam',description:x.dataset.name,quantity:1,unit_price_cents:Number(x.dataset.price||0),total_cents:Number(x.dataset.price||0)}));$$('[data-public-service]:checked').forEach(x=>items.push({item_type:'service',description:x.dataset.name,quantity:1,unit_price_cents:Number(x.dataset.price||0),total_cents:Number(x.dataset.price||0)}));return items};const update=()=>{const items=selectedItems(),total=items.reduce((a,x)=>a+x.total_cents,0);$('#publicQuoteCount').textContent=items.length;if($('#publicQuoteTotal'))$('#publicQuoteTotal').textContent=fmtMoney(total)};$$('[data-public-exam],[data-public-service]').forEach(x=>x.addEventListener('change',update));$('#publicQuoteSearch').addEventListener('input',e=>{const q=e.target.value.toLowerCase().trim();$$('.public-exam-item').forEach(x=>x.classList.toggle('hidden',q&&!x.dataset.search.includes(q)))});
    const localQuote=()=>{const items=selectedItems();return {quote_number:`ONLINE-${today().replaceAll('-','')}`,client_name:$('#publicQuoteName').value||'Cliente avulso',patient_name:$('#publicQuotePatient').value||'',valid_until:'',notes:'Orçamento gerado pelo site HLab Vet.',items,total_cents:items.reduce((a,x)=>a+x.total_cents,0)}};
    $('#publicQuotePdf').addEventListener('click',()=>{const q=localQuote();if(!q.items.length)return toast('Selecione pelo menos um exame ou serviço.','error');printQuote(q)});$('#publicQuoteWhats').addEventListener('click',()=>{const q=localQuote();if(!q.items.length)return toast('Selecione pelo menos um exame ou serviço.','error');const phone=String(d.whatsappPhone||'').replace(/\D/g,''),summary=q.items.map(i=>`• ${i.description}`).join('\n'),text=`Olá! Montei um orçamento no site HLab Vet.${q.patient_name?` Paciente: ${q.patient_name}.`:''}\n\n${summary}${d.showPrices?`\n\nTotal: ${fmtMoney(q.total_cents)}`:''}`;window.open(`https://wa.me/${phone?`55${phone.replace(/^55/,'')}`:''}?text=${encodeURIComponent(text)}`,'_blank','noopener')});update();
  }catch(e){$('#courierView').innerHTML=`<div class="public-quote-shell"><main><div class="public-offline"><h2>Não foi possível abrir o orçamento online</h2><p>${esc(e.message)}</p><a href="/" class="btn primary">Voltar</a></div></main></div>`}
}

async function boot(){
  if(location.pathname.replace(/\/+$/,'')==='/orcamento')return renderPublicQuoteSite();
...
function renderNav(){
  ...
  if(state.me.role==='client'){
    ...
    if(state.me.canMakeQuotes)items.push(['quotes','▧','Orçamentos']);
    items.push(['temperature','▤','Temperaturas'],['password','⚿','Alterar senha']);
  }
  else items=[
    ['dashboard','⌂','Painel'],['requests','▣','Solicitações'],['exam-timing','◷','Tempo de exames'],['cancellations','×','Cancelamentos'],['clients','♙','Clientes'],['couriers','➜','Entregadores'],['receivers','✓','Equipe interna'],['prices','R$','Preços'],['quotes','▧','Orçamentos'],['finance','▦','Financeiro'],['temperature','▤','Temperaturas'],['password','⚿','Alterar senha']
  ];
