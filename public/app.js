...
function renderNav(){
  ...
  if(state.me.role==='client'){
    ...
    ['dashboard','⌂','Painel'],['requests','▣','Solicitações'],['exam-timing','◷','Tempo de exames'],['cancellations','×','Cancelamentos'],['clients','♙','Clientes'],['couriers','➜','Entregadores'],['receivers','✓','Técnicos'],['prices','R$','Preços'],['quotes','▧','Orçamentos'],['finance','▦','Financeiro'],['temperature','▤','Temperaturas'],['password','⚿','Alterar senha']
  ];
...
function quoteSummaryHtml(q,role){
  if(!q)return role==='admin'?`<div class="quote-empty-inline">Nenhum orçamento vinculado. <button class="btn soft small" data-edit-request-quote>Gerar orçamento</button></div>`:'<span class="muted">Nenhum orçamento vinculado.</span>';
  const buttons=`<div class="actions quote-inline-actions"><button class="btn ghost small" data-print-quote-detail="${q.id}">PDF / Imprimir</button><button class="btn soft small" data-share-quote-detail="${q.id}">WhatsApp</button>${role==='admin'?`<button class="btn secondary small" data-edit-request-quote>Editar orçamento</button>`:''}</div>`;
...
function bindAdminRequestActions(id,r){
  ...
  $('#assignBtn')?.addEventListener('click',async()=>{const courierId=Number($('#assignCourier').value);if(!courierId)return toast('Selecione o entregador.','error');const body={courierId};if(state.me.role==='admin'){body.paymentStatus=$('#paymentStatus')?.value||'not_set';if(body.paymentStatus==='collect'){body.paymentMethod=$('#paymentMethod')?.value;body.paymentInstallments=Number($('#paymentInstallments')?.value||1)}}try{const x=await api(`/api/requisitions/${id}/assign`,{json:body});toast(x.message);closeModal();openRequest(id)}catch(e){toast(e.message,'error')}});
...
async function renderPrices(){
  ...
  await loadAdminLists();
  $('#content').innerHTML=`<section class="card price-page-card">
    <div class="price-page-head">
      <div><span class="price-kicker">PRECIFICAÇÃO</span><h3>Tabela de preços dos exames</h3><p>O preço geral é a base do sistema. Quando existir um valor negociado para um cliente, ele substitui somente o preço geral daquele exame.</p></div>
      <button class="btn ghost price-refresh" id="reloadPrices" type="button">↻ Atualizar</button>
    </div>
    <div class="price-toolbar">
      <label class="price-client-filter"><span>Visualizar preços para</span><select id="priceClient"><option value="">Tabela geral</option>${state.clients.filter(c=>c.active).map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select></label>
      <div class="price-toolbar-note"><strong>Preço usado</strong><span>Individual do cliente quando existir; caso contrário, preço geral.</span></div>
    </div>
    <div id="priceList" class="price-list"><div class="empty-state">Carregando preços…</div></div>
  </section>`;
  const moneyValue=c=>c==null?'':(c/100).toFixed(2).replace('.',',');
  const moneyField=(attr,code,value,placeholder)=>`<div class="price-money-field"><span>R$</span><input class="money-input" ${attr}="${code}" value="${value}" placeholder="${placeholder}" inputmode="decimal" autocomplete="off"></div>`;
  const load=async()=>{
    const clientId=$('#priceClient').value,qs=clientId?`?clientId=${encodeURIComponent(clientId)}`:'';
    $('#priceList').innerHTML='<div class="empty-state">Carregando preços…</div>';
    const d=await api(`/api/prices${qs}`),groups=new Map();
    for(const x of d.items){if(!groups.has(x.category))groups.set(x.category,[]);groups.get(x.category).push(x)}
    $('#priceList').innerHTML=[...groups.entries()].map(([cat,items])=>`<section class="price-group">
      <div class="price-group-title"><div><h4>${esc(cat)}</h4><small>${items.length} exame${items.length===1?'':'s'}</small></div></div>
      <div class="table-wrap price-table-wrap"><table class="price-table ${clientId?'has-client-price':''}"><colgroup><col class="col-exam"><col class="col-general">${clientId?'<col class="col-client"><col class="col-used">':''}<col class="col-actions"></colgroup><thead><tr><th>Exame</th><th>Preço geral</th>${clientId?'<th>Preço deste cliente</th><th>Preço usado</th>':''}<th>Ações</th></tr></thead><tbody>${items.map(x=>`<tr class="${x.effectivePriceCents==null?'price-missing':''}"><td class="price-exam-name"><strong>${esc(x.examName)}</strong>${x.effectivePriceCents==null?'<small class="price-status-missing">Sem preço cadastrado</small>':''}</td><td>${moneyField('data-general-price',x.examCode,moneyValue(x.generalPriceCents),'0,00')}</td>${clientId?`<td>${moneyField('data-client-price',x.examCode,moneyValue(x.clientPriceCents),'Usar geral')}</td><td class="price-effective"><strong>${x.effectivePriceCents==null?'—':fmtMoney(x.effectivePriceCents)}</strong><span>${x.source==='client'?'Preço individual':x.source==='general'?'Preço geral':'Pendente'}</span></td>`:''}<td class="price-action-cell"><div class="price-actions"><button class="btn soft small" data-save-general="${x.examCode}" type="button">Salvar geral</button>${clientId?`<button class="btn secondary small" data-save-client="${x.examCode}" type="button">Salvar cliente</button>`:''}</div></td></tr>`).join('')}</tbody></table></div>
    </section>`).join('');
  ...
  $('#content').insertAdjacentHTML('beforeend',`<section class="card services-price-card"><div class="price-page-head"><div><span class="price-kicker">SERVIÇOS</span><h3>Serviços adicionais</h3><p>Cadastre coleta, deslocamento e outros serviços que poderão ser utilizados nos orçamentos.</p></div></div><form id="serviceForm" class="service-form"><label><span>Nome do serviço</span><input name="name" placeholder="Ex.: Coleta domiciliar" required></label><label class="service-description"><span>Descrição</span><input name="description" placeholder="Descrição opcional"></label><label><span>Valor</span><div class="price-money-field"><span>R$</span><input name="price" class="money-input" placeholder="0,00" inputmode="decimal" required></div></label><button class="btn primary service-add-btn">＋ Adicionar serviço</button></form><div id="serviceList">${services.services.length?`<div class="table-wrap service-table-wrap"><table class="service-table"><colgroup><col class="service-name-col"><col><col class="service-value-col"><col class="service-status-col"><col class="service-actions-col"></colgroup><thead><tr><th>Serviço</th><th>Descrição</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>${services.services.map(x=>`<tr><td><strong>${esc(x.name)}</strong></td><td class="service-description-cell">${esc(x.description||'—')}</td><td class="service-value-cell"><strong>${fmtMoney(x.price_cents)}</strong></td><td><span class="badge ${x.active?'recebido':'cancelado'}">${x.active?'Ativo':'Inativo'}</span></td><td><button class="btn soft small" data-edit-service="${x.id}" data-name="${esc(x.name)}" data-description="${esc(x.description||'')}" data-price="${(x.price_cents/100).toFixed(2).replace('.',',')}" data-active="${x.active}">Editar</button></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty-state service-empty">Nenhum serviço cadastrado.</div>'}</div></section>`);
  $('#serviceForm').addEventListener('submit',async e=>{e.preventDefault();const b=Object.fromEntries(new FormData(e.currentTarget));try{const r=await api('/api/services',{json:b});toast(r.message);renderPrices()}catch(er){toast(er.message,'error')}});
  $$('[data-edit-service]').forEach(b=>b.addEventListener('click',()=>{modal(`<h3>Editar serviço</h3><form id="editServiceForm" class="stack"><label>Nome<input name="name" value="${esc(b.dataset.name)}" required></label><label>Descrição<input name="description" value="${esc(b.dataset.description)}"></label><label>Valor<div class="price-money-field"><span>R$</span><input name="price" value="${esc(b.dataset.price)}" inputmode="decimal" required></div></label><label class="inline-check"><input type="checkbox" name="active" value="1" ${b.dataset.active==='1'?'checked':''}> Serviço ativo</label><div class="actions"><button class="btn primary">Salvar</button><button type="button" class="btn ghost" data-close-modal>Cancelar</button></div></form>`);$('#editServiceForm').addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget),body=Object.fromEntries(f);body.active=f.has('active');try{const r=await api(`/api/services/${b.dataset.editService}`,{method:'PATCH',json:body});toast(r.message);closeModal();renderPrices()}catch(er){toast(er.message,'error')}})}));
}


async function renderQuotes(){
  if(!['admin','client'].includes(state.me.role))return navigate('dashboard');
  if(state.me.role==='admin')await loadAdminLists();
  let linkedReq=null,linkedQuote=null;
  if(state.quoteRequisitionId){
    try{
      linkedReq=await api(`/api/requisitions/${state.quoteRequisitionId}`);
      const ql=await api(`/api/quotes?requisitionId=${state.quoteRequisitionId}`);
      if(ql.quotes?.[0]) linkedQuote=(await api(`/api/quotes/${ql.quotes[0].id}`)).quote;
    }catch{}
  }
  const linkedR=linkedReq?.requisition||null;
  const defaultClientId=state.me.role==='client'?Number(state.me.clientId||state.me.client_id||0):Number(linkedR?.client_id||0);
  $('#content').innerHTML=`<div class="quote-layout"><section class="card quote-builder-card"><div class="section-title"><div><h3>${state.me.role==='client'?'Monte seu orçamento':'Gerar orçamento profissional'}</h3><p>${state.me.role==='client'?'Selecione exames e serviços e veja o valor na hora.':'Selecione cliente, exames e serviços. Os valores são calculados pelo cadastro oficial.'}</p></div></div><form id="quoteForm" class="stack"><div class="form-grid">${state.me.role==='admin'?`<label class="field span2">Cliente<select id="quoteClient" required><option value="">Selecione</option>${state.clients.filter(c=>c.active).map(c=>`<option value="${c.id}" ${Number(c.id)===defaultClientId?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label>`:''}<label class="field span2">Paciente / animal (opcional)<input id="quotePatient" value="${esc(linkedQuote?.patient_name||linkedR?.patient_name||'')}"></label><label class="field">Validade<input id="quoteValidity" type="date" value="${esc(linkedQuote?.valid_until||'')}"></label><label class="field span4">Observações<textarea id="quoteNotes" placeholder="Condições, orientações ou observações do orçamento">${esc(linkedQuote?.notes||'')}</textarea></label></div><div id="quoteCatalog" class="quote-catalog"><div class="empty-state">${state.me.role==='admin'&&!defaultClientId?'Selecione o cliente para carregar os valores.':'Carregando valores...'}</div></div><div class="quote-total-bar"><div><small>TOTAL DO ORÇAMENTO</small><strong id="quoteLiveTotal">R$ 0,00</strong></div><button class="btn primary" type="submit">Salvar orçamento</button></div></form></section><aside class="card quote-history"><div class="section-title"><div><h3>Orçamentos salvos</h3><p>Abra para imprimir, enviar no WhatsApp ou usar em uma solicitação.</p></div></div><div id="quoteHistory"></div></aside></div>`;
  const clientSelect=$('#quoteClient');let catalog=null,currentQuote=null;
  const clientId=()=>state.me.role==='client'?Number(state.me.clientId||state.me.client_id||0):Number(clientSelect?.value||0);
  const calc=()=>{let total=0;$$('[data-q-exam]:checked').forEach(i=>total+=Number(i.dataset.price||0));$$('[data-q-service]:checked').forEach(i=>{const qty=Math.max(1,Number($(`[data-q-qty="${i.dataset.qService}"]`)?.value||1));total+=Number(i.dataset.price||0)*qty});$('#quoteLiveTotal').textContent=fmtMoney(total)};
  const renderCatalog=async()=>{const cid=clientId();if(!cid){$('#quoteCatalog').innerHTML='<div class="empty-state">Selecione o cliente para carregar os valores.</div>';return}try{catalog=await api(`/api/quotes/catalog?clientId=${cid}`);const groups=new Map();for(const x of catalog.exams){if(!groups.has(x.category))groups.set(x.category,[]);groups.get(x.category).push(x)}$('#quoteCatalog').innerHTML=`<div class="quote-section-title">Exames</div><div class="quote-exam-grid">${[...groups.entries()].map(([cat,items])=>`<section class="quote-group"><h4>${esc(cat)}</h4>${items.map(x=>`<label class="quote-choice ${x.priceCents==null?'disabled':''}"><input type="checkbox" data-q-exam="${x.examCode}" data-price="${x.priceCents||0}" ${x.priceCents==null?'disabled':''}><span>${esc(x.examName)}</span><strong>${x.priceCents==null?'Sem preço':fmtMoney(x.priceCents)}</strong></label>`).join('')}</section>`).join('')}</div><div class="quote-section-title">Serviços adicionais</div>${catalog.services.length?`<div class="quote-service-list">${catalog.services.map(x=>`<div class="quote-service-row"><label><input type="checkbox" data-q-service="${x.id}" data-price="${x.price_cents}"><span><b>${esc(x.name)}</b>${x.description?`<small>${esc(x.description)}</small>`:''}</span></label><strong>${fmtMoney(x.price_cents)}</strong><input class="quote-qty" data-q-qty="${x.id}" type="number" min="1" max="999" value="1" title="Quantidade"></div>`).join('')}</div>`:'<div class="muted">Nenhum serviço adicional cadastrado.</div>'}`;$$('[data-q-exam],[data-q-service],[data-q-qty]').forEach(x=>x.addEventListener('change',calc));if(linkedReq||linkedQuote){const selected=new Set(linkedQuote?linkedQuote.items.filter(i=>i.item_type==='exam').map(i=>i.item_ref):(linkedReq.exams||[]).map(e=>e.exam_code));$$('[data-q-exam]').forEach(i=>i.checked=selected.has(i.dataset.qExam));if(linkedQuote){for(const item of linkedQuote.items.filter(i=>i.item_type==='service')){const cb=$(`[data-q-service="${item.item_ref}"]`),qty=$(`[data-q-qty="${item.item_ref}"]`);if(cb)cb.checked=true;if(qty)qty.value=item.quantity}}calc()} }catch(e){$('#quoteCatalog').innerHTML=`<div class="warning-box">${esc(e.message)}</div>`}};
  const loadHistory=async()=>{const cid=clientId(),qs=new URLSearchParams();if(cid)qs.set('clientId',cid);if(state.quoteRequisitionId)qs.set('requisitionId',state.quoteRequisitionId);const d=await api(`/api/quotes?${qs}`);$('#quoteHistory').innerHTML=d.quotes.length?d.quotes.map(q=>`<article class="quote-history-item"><div><strong>${esc(q.quote_number)}</strong><small>${esc(q.patient_name||'Sem paciente')} • ${fmtDateTime(q.updated_at)}</small></div><b>${fmtMoney(q.total_cents)}</b><div class="actions"><button class="btn ghost small" data-q-open="${q.id}">Abrir</button><button class="btn soft small" data-q-print="${q.id}">PDF</button><button class="btn soft small" data-q-share="${q.id}">WhatsApp</button>${state.me.role==='client'&&!q.requisition_id?`<button class="btn primary small" data-q-request="${q.id}">Criar solicitação</button>`:''}</div></article>`).join(''):'<div class="empty-state">Nenhum orçamento salvo.</div>';$$('[data-q-open]').forEach(b=>b.addEventListener('click',()=>openQuoteModal(Number(b.dataset.qOpen))));$$('[data-q-print]').forEach(b=>b.addEventListener('click',()=>printQuoteById(Number(b.dataset.qPrint))));$$('[data-q-share]').forEach(b=>b.addEventListener('click',()=>shareQuoteWhatsApp(Number(b.dataset.qShare))));$$('[data-q-request]').forEach(b=>b.addEventListener('click',()=>useQuoteForRequest(Number(b.dataset.qRequest))))};
  clientSelect?.addEventListener('change',async()=>{await renderCatalog();await loadHistory()});
  $('#quoteForm').addEventListener('submit',async e=>{e.preventDefault();const cid=clientId();if(!cid)return toast('Selecione o cliente.','error');const examCodes=$$('[data-q-exam]:checked').map(x=>x.dataset.qExam),services=$$('[data-q-service]:checked').map(x=>({id:Number(x.dataset.qService),quantity:Number($(`[data-q-qty="${x.dataset.qService}"]`)?.value||1)}));try{const x=await api('/api/quotes',{json:{quoteId:linkedQuote?.id||null,clientId:cid,requisitionId:state.quoteRequisitionId||null,patientName:$('#quotePatient').value,validUntil:$('#quoteValidity').value,notes:$('#quoteNotes').value,examCodes,services}});toast(x.message);currentQuote=x.quote;linkedQuote=x.quote;await loadHistory();state.quoteRequisitionId=null;openQuoteModal(x.quote.id)}catch(er){toast(er.message,'error')}});
  if(defaultClientId||state.me.role==='client')await renderCatalog();await loadHistory();
}
async function openQuoteModal(id){try{const d=await api(`/api/quotes/${id}`),q=d.quote;modal(`<div class="quote-preview"><div class="quote-preview-head"><img src="/assets/hlabvet-logo.png" alt="HLabVet"><div><small>ORÇAMENTO</small><h2>${esc(q.quote_number)}</h2></div><strong>${fmtMoney(q.total_cents)}</strong></div><div class="quote-preview-meta"><div><small>CLIENTE</small><b>${esc(q.client_name)}</b></div><div><small>PACIENTE</small><b>${esc(q.patient_name||'—')}</b></div><div><small>VALIDADE</small><b>${q.valid_until?fmtDate(q.valid_until):'—'}</b></div></div><div class="table-wrap"><table><thead><tr><th>Item</th><th>Qtd.</th><th>Unitário</th><th>Total</th></tr></thead><tbody>${q.items.map(i=>`<tr><td>${esc(i.description)}<br><small>${i.item_type==='exam'?'Exame':'Serviço'}</small></td><td>${i.quantity}</td><td>${fmtMoney(i.unit_price_cents)}</td><td><strong>${fmtMoney(i.total_cents)}</strong></td></tr>`).join('')}</tbody><tfoot><tr><td colspan="3"><strong>Total</strong></td><td><strong>${fmtMoney(q.total_cents)}</strong></td></tr></tfoot></table></div>${q.notes?`<div class="quote-notes"><b>Observações</b><p>${esc(q.notes)}</p></div>`:''}<div class="actions" style="margin-top:16px"><button class="btn primary" id="quotePdfBtn">PDF / Imprimir</button><button class="btn soft" id="quoteWhatsBtn">Enviar no WhatsApp</button>${state.me.role==='client'&&!q.requisition_id?'<button class="btn secondary" id="quoteRequestBtn">Criar solicitação</button>':''}<button class="btn ghost" data-close-modal>Fechar</button></div></div>`);$('#quotePdfBtn').addEventListener('click',()=>printQuote(q));$('#quoteWhatsBtn').addEventListener('click',()=>shareQuoteWhatsApp(q.id));$('#quoteRequestBtn')?.addEventListener('click',()=>{closeModal();useQuoteForRequest(q.id)})}catch(e){toast(e.message,'error')}}
async function useQuoteForRequest(id){try{const d=await api(`/api/quotes/${id}`),q=d.quote;state.pendingQuote={quoteId:q.id,patientName:q.patient_name||'',examCodes:q.items.filter(i=>i.item_type==='exam').map(i=>i.item_ref)};navigate('new-request')}catch(e){toast(e.message,'error')}}
