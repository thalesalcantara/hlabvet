...
async function renderPublicWebsite(focusQuote=false){
  ...
  try{
    ...
    const hashMap={inicio:'home',home:'home',cotacao:'cotacao',servicos:'servicos',parceiros:'parceiros',localizacao:'localizacao',trabalhe:'trabalhe',contato:'contato'};
    const initialPublicTab=focusQuote?'cotacao':(hashMap[location.hash.replace('#','')]||'home');
    openPublicTab(initialPublicTab,{updateUrl:false});
    if(popupOffer&&$('#publicPromoPopup')){
      const popup=$('#publicPromoPopup'),closeBtn=$('#publicPromoClose'),count=$('#publicPromoCountdown');let remaining=Math.max(1,Number(popupOffer.popup_seconds||5)),closed=false;
      const closePromo=()=>{if(closed)return;closed=true;popup.classList.add('closing');setTimeout(()=>popup.remove(),220)};
      closeBtn?.addEventListener('click',closePromo);
      const timer=setInterval(()=>{remaining--;if(count)count.textContent=Math.max(0,remaining);if(remaining<=0){clearInterval(timer);closePromo()}},1000);
    }
