...
async function renderPrices(){
  ...
  $('#content').insertAdjacentHTML('beforeend',`<div class="card" style="margin-top:16px"><div class="section-title"><div><h3>Serviços adicionais</h3><p>Cadastre coleta, deslocamento ou qualquer outro serviço que possa entrar no orçamento.</p></div></div><form id="serviceForm" class="service-form"><input name="name" placeholder="Nome do serviço" required><input name="description" placeholder="Descrição opcional"><input name="price" class="money-input" placeholder="0,00" required><button class="btn primary">Adicionar serviço</button></form><div id="serviceList">${services.services.length?`<div class="table-wrap"><table><thead><tr><th>Serviço</th><th>Descrição</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>${services.services.map(x=>`<tr><td><strong>${esc(x.name)}</strong></td><td>${esc(x.description||'—')}</td><td>${fmtMoney(x.price_cents)}</td><td><span class="badge ${x.active?'recebido':'cancelado'}">${x.active?'Ativo':'Inativo'}</span></td><td><button class="btn soft small" data-edit-service="${x.id}" data-name="${esc(x.name)}" data-description="${esc(x.description||'')}" data-price="${(x.price_cents/100).toFixed(2).replace('.',',')}" data-active="${x.active}">Editar</button></td></tr>`).join('')}</tbody></table></div>`:'<div class="empty-state">Nenhum serviço cadastrado.</div>'}</div></div>`);
