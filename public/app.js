...
const state = { me:null, catalog:null, page:null, clients:[], couriers:[], receivers:[], alerts:[] };
...
let alertPollTimer=null, alertBeepTimer=null, audioCtx=null;
...
function stopLabAlerts(){
  clearInterval(alertPollTimer);clearInterval(alertBeepTimer);alertPollTimer=null;alertBeepTimer=null;state.alerts=[];$('#labAlertDock')?.remove();
}
function renderAlertDock(){
  let dock=$('#labAlertDock');
  if(!state.alerts.length){dock?.remove();return;}
  ...
  dock.innerHTML=`<div class="lab-alert-head">🔔 ${state.alerts.length} solicitação${state.alerts.length>1?'ões':''} aguardando aceite</div>${state.alerts.map(a=>`<div class="lab-alert-item"><div><strong>${esc(a.client_name)}</strong><small>${esc(a.protocol)} • ${esc(a.patient_name)}${a.request_kind==='scheduled'?` • agendada ${fmtDateTime(a.scheduled_at)}`:''}</small></div><button class="btn primary small" data-accept-alert="${a.id}">Aceitar</button></div>`).join('')}`;
...
async function pollLabAlerts(){
  ...
  try{const d=await api('/api/alerts');state.alerts=d.alerts||[];renderAlertDock();if(state.alerts.length){if(!alertBeepTimer){beep();alertBeepTimer=setInterval(beep,1800)}}else{clearInterval(alertBeepTimer);alertBeepTimer=null}}catch{}
}
...
function renderNav(){
  let items;
  if(state.me.role==='client') items=[
    ['dashboard','⌂','Painel'],['new-request','＋','Nova solicitação'],['requests','▣','Meus exames'],['stamp','▧','Meu carimbo'],['password','⚿','Alterar senha']
  ];
  else if(state.me.role==='staff') items=[
    ['dashboard','⌂','Painel'],['requests','▣','Solicitações'],['temperature','▤','Temperaturas'],['password','⚿','Alterar senha']
  ];
  else items=[
    ['dashboard','⌂','Painel'],['requests','▣','Solicitações'],['clients','♙','Clientes'],['couriers','➜','Entregadores'],['receivers','✓','Técnicos'],['temperature','▤','Temperaturas'],['audit','◴','Auditoria'],['password','⚿','Alterar senha']
  ];
...
async function openClientResults(id){
  try{
    ...
    modal(`<div class="detail-head"><div><h3>Resultado • ${esc(r.patient_name)}</h3><p class="muted">${esc(r.protocol)} • ${esc(r.client_name)}</p></div></div><h4>Exames solicitados</h4><div>${d.exams.map(x=>`<span class="badge" style="margin:2px">${esc(x.exam_name)}</span>`).join('')}</div><h4 style="margin-top:18px">Arquivos de resultado</h4>${d.files.length?d.files.map(f=>`<div class="file-card"><div><strong>${esc(f.original_name)}</strong><small>${fmtBytes(f.size_bytes)} • ${fmtDateTime(f.created_at)}</small></div><div class="actions"><a class="btn soft small" target="_blank" href="/api/results/${f.id}/view">Visualizar</a><a class="btn secondary small" href="/api/results/${f.id}/download">Baixar</a><button class="btn ghost small" data-print-result="${f.id}">Imprimir</button></div></div>`).join(''):'<div class="empty-state">Resultado ainda não disponível.</div>'}<div class="actions" style="margin-top:16px"><button class="btn ghost" data-close-modal>Fechar</button></div>`);
    $$('[data-print-result]').forEach(b=>b.addEventListener('click',()=>{const w=window.open(`/api/results/${b.dataset.printResult}/view`,'_blank');if(!w)toast('Libere pop-ups para imprimir.','error');else toast('O resultado foi aberto. Use a opção Imprimir do navegador.')}));
...
async function openRequest(id){
  ...
  const resultHtml=d.files.length?d.files.map(f=>`<div class="file-card"><div><strong>${esc(f.original_name)}</strong><small>${fmtBytes(f.size_bytes)} • ${fmtDateTime(f.created_at)}</small></div><div class="actions"><a class="btn soft small" target="_blank" href="/api/results/${f.id}/view">Visualizar</a><a class="btn secondary small" href="/api/results/${f.id}/download">Baixar</a>${state.me.role==='client'?`<button class="btn ghost small" data-print-result="${f.id}">Imprimir</button>`:''}</div></div>`).join(''):'<p class="muted">Nenhum resultado enviado ainda.</p>';
  modal(`<div class="detail-head"><div><h3>${esc(r.protocol)} • ${esc(r.patient_name)}</h3><p class="muted">${esc(r.client_name)} • criado em ${fmtDateTime(r.created_at)}${r.request_kind==='scheduled'?` • agendado para ${fmtDateTime(r.scheduled_at)}`:''}</p></div><span class="badge ${r.status}">${esc(STATUS[r.status])}</span></div>
    ...
    <div class="actions" style="margin-top:18px">${state.me.role==='admin'?'<button class="btn ghost" id="printThis">Imprimir / Salvar em PDF</button>':''}<button class="btn ghost" data-close-modal>Fechar</button></div>`);
...
async function renderCouriers(){
  ...
  $('#content').innerHTML=`<div class="card"><p class="muted">Cada entregador possui um link privado e um termômetro identificado como TER-001, TER-002 etc. Se um entregador for substituído, você pode editar o nome mantendo o mesmo termômetro; o histórico antigo não muda.</p><div class="table-wrap"><table><thead><tr><th>Entregador</th><th>Telefone</th><th>Termômetro</th><th>Link</th><th>Status</th><th>Ações</th></tr></thead><tbody>${state.couriers.map(c=>`<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(c.phone||'')}</td><td><strong>${esc(c.thermometer_code||'—')}</strong></td><td>••••${esc(c.token_last4||'')}</td><td><span class="badge ${c.active?'concluido':'cancelado'}">${c.active?'Ativo':'Inativo'}</span></td><td><div class="actions"><button class="btn soft small" data-edit-courier="${c.id}">Editar</button><button class="btn secondary small" data-link-courier="${c.id}">Gerar novo link</button></div></td></tr>`).join('')}</tbody></table></div></div>`;
...
async function renderReceivers(){
  const d=await api('/api/technicians');state.receivers=d.receivers||[];$('#topActions').innerHTML=`<button class="btn primary" id="addReceiver">＋ Técnico</button>`;
  $('#content').innerHTML=`<div class="card"><p class="muted">Cada técnico possui login próprio. Quando ele registra o recebimento estando logado, o nome do técnico é gravado automaticamente na ficha e no histórico.</p><div class="table-wrap"><table><thead><tr><th>Técnico</th><th>Usuário</th><th>Local padrão</th><th>Status</th><th>Ações</th></tr></thead><tbody>${state.receivers.map(r=>`<tr><td><strong>${esc(r.name)}</strong></td><td>${esc(r.username_display||'Sem login')}</td><td>${esc(r.location||'HLab Vet')}</td><td><span class="badge ${r.active?'concluido':'cancelado'}">${r.active?'Ativo':'Inativo'}</span></td><td><div class="actions"><button class="btn soft small" data-edit-receiver="${r.id}">Editar</button>${r.user_id?`<button class="btn ghost small" data-reset-tech="${r.id}">Alterar senha</button>`:''}</div></td></tr>`).join('')}</tbody></table></div></div>`;
  $('#addReceiver').addEventListener('click',()=>receiverForm());$$('[data-edit-receiver]').forEach(b=>b.addEventListener('click',()=>receiverForm(state.receivers.find(x=>x.id===Number(b.dataset.editReceiver)))));$$('[data-reset-tech]').forEach(b=>b.addEventListener('click',()=>resetTechnician(Number(b.dataset.resetTech))));
}
